...
function gml_builder(l_pg, l_src) constructor {
	...
	static h_build_expr = function(l_flags) {
		...
		switch (l_tk0.__enumIndex__/* gml_token */) {
			...
			case 11/* ident */:
				...
				break;
			case 25/* arg_const */: self.h_out_node = gml_node_arg_const(l_tk0.h_d, l_tk0.h_i); break;
			case 15/* un_op */:
			...
			case 19/* par_open */:
				...
				} else if (self.h_tokens[self.h_offset].__enumIndex__/* gml_token */ == 20/* par_close */) {
					self.h_offset += 1;
			...
			case 23/* cub_open */:
				...
				var l__g = self.h_tokens[self.h_offset];
				var l_tmp;
				if (l__g.__enumIndex__/* gml_token */ == 24/* cub_close */) l_tmp = true; else l_tmp = false;
				if (l_tmp) {
					self.h_offset += 1;
					...
					switch (l__g.__enumIndex__/* gml_token */) {
						case 14/* cstring */:
							...
							switch (self.h_tokens[self.h_offset].__enumIndex__/* gml_token */) {
								case 4/* comma */:
									self.h_offset += 1;
									if (self.h_tokens[self.h_offset].__enumIndex__/* gml_token */ == 24/* cub_close */) {
										self.h_offset += 1;
										l_proc = false;
									}
									break;
			...
			case 10/* keyword */:
				switch (l_tk0.h_kw) {
					case 0:
						...
						if (self.h_offset >= self.h_length) {
							...
							if (self.h_offset >= self.h_length) {
								...
								if (l__g1.__enumIndex__/* gml_token */ == 11/* ident */) {
									self.h_offset += 1;
					...
					case 26:
						...
						} else if (self.h_tokens[self.h_offset].__enumIndex__/* gml_token */ == 21/* sqb_open */) {
							self.h_offset += 1;
						...
						} else if (self.h_tokens[self.h_offset].__enumIndex__/* gml_token */ == 22/* sqb_close */) {
							self.h_offset += 1;
					...
					case 25:
						...
						if (l__g1.__enumIndex__/* gml_node */ == 22/* call */) self.h_out_node = gml_node_construct(l__g1.h_d, l__g1.h_x, l__g1.h_args); else return self.h_expect("a callable value after `new`", l_tk0);
						break;
					case 24:
						var l_d = l_tk0.h_d;
						...
						} else if (self.h_tokens[self.h_offset].__enumIndex__/* gml_token */ == 19/* par_open */) {
							self.h_offset += 1;
						...
						if (self.h_offset < self.h_length) {
							l_tk = self.h_tokens[self.h_offset];
							var l_tmp;
							if (l_tk.__enumIndex__/* gml_token */ == 11/* ident */) l_tmp = l_tk.h_id == "constructor"; else l_tmp = false;
						...
						l_tk = self.h_tokens[self.h_offset];
						if (l_tk.__enumIndex__/* gml_token */ == 23/* cub_open */) var l__g1 = l_tk.h_d; else return self.h_expect("a `{`", self.h_tokens[self.h_offset]);
						self.h_build_line();
			...
			switch (l_tk.__enumIndex__/* gml_token */) {
				...
				case 5/* period */:
					if ((l_flags & 2) == 0) {
						self.h_offset += 1;
						var l__g = self.h_tokens[self.h_offset];
						if (l__g.__enumIndex__/* gml_token */ == 11/* ident */) {
							l_s = l__g.h_id;
							self.h_offset += 1;
		...
		switch (l_tk.__enumIndex__/* gml_token */) {
				...
				switch (l_tk.h_kw) {
					case 1:
						...
						while (self.h_offset < self.h_length) {
							...
							if (l_tk2.__enumIndex__/* gml_token */ == 11/* ident */) {
								l_d = l_tk2.h_d;
								l_s = l_tk2.h_id;
					...
					case 2:
						...
						while (l_proc && self.h_offset < self.h_length) {
							...
							switch (l_tk2.__enumIndex__/* gml_token */) {
								case 10/* keyword */: if (l_tk2.h_kw == 2) var l_d1 = l_tk2.h_d; else return self.h_expect("a variable name", l_tk2); break;
								case 11/* ident */:
									if (self.h_offset >= self.h_length) return self.h_error_at("Expected a variable value", self.h_source.h_get_eof());
									l_tk = self.h_tokens[self.h_offset];
									if (l_tk.__enumIndex__/* gml_token */ == 18/* set_op */) {
										if (l_tk.h_op == -1) {
											self.h_offset += 1;
											if (self.h_build_expr(0)) return true;
					...
					case 3:
						...
						} else if (self.h_tokens[self.h_offset].__enumIndex__/* gml_token */ == 23/* cub_open */) {
							self.h_offset += 1;
							...
							switch (l__g1.__enumIndex__/* gml_token */) {
								...
								case 11/* ident */:
									if (l_sep) {
										...
										if (l__g3.__enumIndex__/* gml_token */ == 18/* set_op */) {
											if (l__g3.h_op == -1) {
												self.h_offset += 1;
												if (self.h_build_expr(0)) return true;
											} else self.h_out_node = undefined;
					...
					case 4:
						...
						var l__g1 = self.h_tokens[self.h_offset];
						if (l__g1.__enumIndex__/* gml_token */ == 10/* keyword */) {
					...
					case 7:
						...
						var l__g1 = self.h_tokens[self.h_offset];
						if (l__g1.__enumIndex__/* gml_token */ == 10/* keyword */) switch (l__g1.h_kw) {
							case 8: case 9: break;
									...
									switch (l__g1.h_kw) {
										case 8:
											...
											while (self.h_offset < self.h_length) {
												...
												if (l__g5.__enumIndex__/* gml_token */ == 10/* keyword */) {
													if (l__g5.h_kw == 8) {
														self.h_offset += 1;
														if (self.h_build_expr(0)) return true;
														if (self.h_offset >= self.h_length) {
					...
					case 14:
						...
						if (self.h_tokens[self.h_offset].__enumIndex__/* gml_token */ == 19/* par_open */) {
							self.h_offset += 1;
							l_proc = true;
						} else l_proc = false;
						...
						if (self.h_offset >= self.h_length) return self.h_error_at("Expected for-loop post-action", self.h_source.h_get_eof());
						if (l_proc) {
							if (self.h_tokens[self.h_offset].__enumIndex__/* gml_token */ == 20/* par_close */) {
								self.h_offset += 1;
								l_proc = false;
						...
						if (l_proc) {
							if (self.h_tokens[self.h_offset].__enumIndex__/* gml_token */ == 20/* par_close */) self.h_offset += 1; else return self.h_expect("a closing parenthesis", self.h_tokens[self.h_offset]);
						}
						if (self.h_build_line()) return true;
					...
					case 18:
						...
						if (self.h_offset < self.h_length) {
							var l__g1 = self.h_tokens[self.h_offset];
							switch (l__g1.__enumIndex__/* gml_token */) {
								case 24/* cub_close */: self.h_out_node = gml_node_exit(l__g1.h_d); break;
			...
			case 1/* macro */:
				...
				if (l__g.__enumIndex__/* gml_token */ == 11/* ident */) {
					var l_s1 = l__g.h_id;
					var l_add;
					if (self.h_tokens[self.h_offset].__enumIndex__/* gml_token */ == 6/* colon */) {
						self.h_offset += 1;
						l_add = l_s1 == live_config;
						var l__g = self.h_tokens[self.h_offset++];
						if (l__g.__enumIndex__/* gml_token */ == 11/* ident */) l_s1 = l__g.h_id; else return self.h_error("Expected a macro name", self.h_tokens[self.h_offset]);
					} else l_add = true;
	...
	static h_build_outer = function(l_name, l_namedArgs, l_namedArgc, l_isFunc, l_prefixStatements) {
		...
		if (l_isFunc) {
			...
			if (l__g.__enumIndex__/* gml_node */ == 102/* block */) l_nodes = l__g.h_nodes; else l_nodes = [l_scr.h_node];
	...
	static h_build_script_args = function() {
		...
		var l__g = self.h_tokens[self.h_offset];
		var l_tmp;
		if (l__g.__enumIndex__/* gml_token */ == 20/* par_close */) l_tmp = true; else l_tmp = false;
		if (l_tmp) {
			self.h_offset += 1;
		} else while (l_proc && self.h_offset < self.h_length) {
			var l__g = self.h_tokens[self.h_offset];
			if (l__g.__enumIndex__/* gml_token */ == 11/* ident */) {
				...
				if (l__g2.__enumIndex__/* gml_token */ == 18/* set_op */) {
					if (l__g2.h_op == -1) {
			...
			switch (l_tk.__enumIndex__/* gml_token */) {
				case 0/* header */:
					...
					if (!(l_tk.h_lb || self.h_offset >= self.h_length)) {
						var l__g1 = self.h_tokens[self.h_offset];
						var l_tmp;
						if (l__g1.__enumIndex__/* gml_token */ == 19/* par_open */) l_tmp = true; else l_tmp = false;
						if (l_tmp) {
							self.h_offset += 1;
							if (self.h_build_script_args()) return true;
				...
				case 10/* keyword */:
					if (l_tk.h_kw == 24) {
						...
						var l_nextName1;
						l_tk = self.h_tokens[self.h_offset];
						if (l_tk.__enumIndex__/* gml_token */ == 11/* ident */) {
						...
						l_tk = self.h_tokens[self.h_offset++];
						var l_tmp1;
						if (l_tk.__enumIndex__/* gml_token */ == 19/* par_open */) l_tmp1 = true; else l_tmp1 = false;
						if (!l_tmp1) return self.h_expect("an opening `(`", l_tk);
						if (self.h_build_script_args()) return true;
						if (self.h_offset < self.h_length) {
							l_tk = self.h_tokens[self.h_offset];
							var l_tmp2;
							if (l_tk.__enumIndex__/* gml_token */ == 11/* ident */) l_tmp2 = l_tk.h_id == "constructor"; else l_tmp2 = false;
